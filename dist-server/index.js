...
app.post('/api/menu', authenticateToken, (req, res) => {
    ...
    try {
        const result = db.prepare('INSERT INTO menu_items (name, description, price, category, image_url, available) VALUES (?, ?, ?, ?, ?, ?)').run(name, description, price, category, image_url, available ? 1 : 0);
...
app.put('/api/menu/:id', authenticateToken, (req, res) => {
    const { name, description, price, category, image_url, available } = req.body;
    try {
        db.prepare('UPDATE menu_items SET name = ?, description = ?, price = ?, category = ?, image_url = ?, available = ? WHERE id = ?').run(name, description, price, category, image_url, available ? 1 : 0, req.params.id);
...
app.post('/api/orders', (req, res) => {
    ...
    try {
        const result = db.prepare('INSERT INTO orders (customer_name, customer_phone, customer_address, items, total_amount, status) VALUES (?, ?, ?, ?, ?, ?)').run(customer_name, customer_phone, customer_address, JSON.stringify(items), total_amount, 'pending');
...
app.post('/api/reservations', (req, res) => {
    ...
    try {
        const result = db.prepare('INSERT INTO reservations (customer_name, customer_phone, customer_email, date, time, guests, special_requests, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(customer_name, customer_phone, customer_email, date, time, guests, special_requests, 'pending');
        const r = db.prepare('SELECT * FROM reservations WHERE id = ?').get(result.lastInsertRowid);
        res.json({
            id: r.id,
            customerName: r.customer_name,
            customerPhone: r.customer_phone,
...
app.put('/api/reservations/:id', authenticateToken, (req, res) => {
    ...
    try {
        ...
        if (table_id)
            db.prepare('UPDATE reservations SET table_id = ? WHERE id = ?').run(table_id, req.params.id);
...
app.get('/api/reviews', (req, res) => {
    try {
        const approved = db.prepare('SELECT * FROM reviews WHERE approved = 1 ORDER BY created_at DESC').all();
...
app.get('/api/admin/reviews', authenticateToken, (req, res) => {
    try {
        const all = db.prepare('SELECT * FROM reviews ORDER BY created_at DESC').all();
        const totalRating = all.reduce((sum, r) => sum + r.rating, 0);
        const averageRating = all.length > 0 ? totalRating / all.length : 0;
...
app.post('/api/reviews', upload.single('image'), async (req, res) => {
    try {
        ...
        const result = db.prepare('INSERT INTO reviews (customer_name, rating, review_text, image_url, approved) VALUES (?, ?, ?, ?, ?)').run(req.body.customer_name, req.body.rating, req.body.review_text, imageUrl, 0);
...
app.get('/api/admin/stats', authenticateToken, (req, res) => {
    try {
        ...
        const ordersResult = db.prepare(`SELECT COUNT(*) as count FROM orders ${dateFilter}`).get();
        const revenueResult = db.prepare(`SELECT SUM(total_amount) as total FROM orders ${dateFilter} ${statusFilter}`.replace('WHERE AND', 'WHERE')).get();
        const reviewsResult = db.prepare(`SELECT COUNT(*) as count, AVG(rating) as avg FROM reviews ${dateFilter}`).get();
        res.json({
